...
description: Turn any design style into a polished HTML slide deck and export it as a high-quality PDF. Triggers when users say things like "make me a nice PPT", "create slides based on X style", "build a deck", "generate presentation", "帮我做PPT", "做幻灯片", "制作演示文稿", "生成slides", "做个deck". Supports four input types: (1) Upload a .pptx template file — extract its colors, fonts and layout via python-pptx; (2) Share an image or screenshot of a design; (3) Provide a website URL to clone its visual style; (4) Describe a style in words (e.g. "Apple style", "Notion minimal", "Linear dark"). Output: pixel-sharp 1440×900 slides exported to PDF via Puppeteer, production-quality design.
---
...
## Step 4: Export PDF via Puppeteer
...
| Chinese text renders as Song/Ming | Add `'PingFang SC', 'Microsoft YaHei', 'Noto Sans SC'` to font stack for cross-platform Chinese support |
